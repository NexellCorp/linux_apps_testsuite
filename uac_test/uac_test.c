 ...
 *      Raw Data
 *
 *  Copyright (c) by Jaroslav Kysela <hsjung@nexell.co.kr>
 *
 *
...
#define SND_TEST_VERSION_STR	"1.0.0"
#ifndef LLONG_MAX
...
{
	printf("uac-test: version " SND_TEST_VERSION_STR " by Hyunseok Jung <hsjung@nexell.co.kr>\n");
}
...
void printfmtmask(const snd_pcm_format_mask_t *fmask)
{
  ...
  for( fmt= 0; fmt <= SND_PCM_FORMAT_LAST; ++fmt )
    if( snd_pcm_format_mask_test(fmask, (snd_pcm_format_t)fmt) ) {
      if( prevformat )
	fprintf(stderr, ", ");
      fprintf(stderr, "%s", snd_pcm_format_name((snd_pcm_format_t)fmt));
      prevformat= 1;
    }
  if( !prevformat )
    fprintf(stderr, "(none)");
...
int main(int argc, char *argv[])
{
	...
	if (test_all) {
		...
		snd_pcm_format_t tmp_format, format;
		unsigned int rates, rrate, rrate_min, rrate_max;
		unsigned int channels, cchan, chan_min, chan_max;
		snd_pcm_hw_params_alloca(&params);
		snd_pcm_format_mask_alloca(&m);
		snd_pcm_hw_params_any(handle, params);
		snd_pcm_hw_params_get_format(params, &tmp_format);
		format = tmp_format;
		fprintf(stderr, "supported format %s(%d)\n", snd_pcm_format_name(format), format);
	        //snd_pcm_hw_params_get_format_mask(params, m);
		//printfmtmask(m);
#if 0
	        if (m) { /* bitmask, refer to: SNDRV_PCM_FORMAT_* */
	            ...
	            fprintf(stderr, "   Format[0]:\t%#08x\n", m.bits[0]);
	            fprintf(stderr, "   Format[1]:\t%#08x\n", m.bits[1]);
	            ...
	            for (k = 0; k < 2; ++k) {
	                for (j = 0; j < bitcount; ++j) {
        	            ...
        	            if (m->bits[k] & (1 << j)) {
	                        name = snd_pcm_format_name(j + k*bitcount);
	                        if (name) {
	                            if (count++ == 0) {
		...
		fprintf(stderr, "supported rate - %x(%u)\n", rrate, rrate);
		snd_pcm_hw_params_get_rate_min(params, &rrate, 0);
		rrate_min = rrate;
		fprintf(stderr, "supported min rate - %u\n", rrate_min);
		snd_pcm_hw_params_get_rate_max(params, &rrate, 0);
		rrate_max = rrate;
		fprintf(stderr, "supported max rate - %u\n", rrate_max);
		snd_pcm_hw_params_get_channels_min(params, &cchan);
		chan_min = cchan;
		fprintf(stderr, "supported min channels - %d\n", chan_min);
		snd_pcm_hw_params_get_channels_max(params, &cchan);
		chan_max = cchan;
		fprintf(stderr, "supported max channels - %d\n", chan_max);

		snd_pcm_close(handle);
		handle = NULL;

		if (timelimit == 0)
			timelimit = 5; // default min 5s
		
		for (channels = chan_min; channels <= chan_max; channels++) {
			for (rates = rrate_min; rates <= rrate_max;rates+=8000) {
				char tmp[10];
				sprintf(tmp, "test_%s_%dch_%dhz.wav",
					snd_pcm_format_name(format), channels, rates);
				...
				snd_pcm_close(handle);
				handle = NULL;
				fprintf(stderr, "channels %d chan_max %d\n", channels, chan_max);
				//fprintf(stderr, "rates %d rate_max %d\n", rates, rrate_max);
				//if (rates > rrate_max)
				//	goto __end;
			}
		}
