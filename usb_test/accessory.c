...
accessory:
	...
	if ((acc->pid != AOA_AUDIO_ADB_PID) && (acc->pid != AOA_AUDIO_PID)) {
		...
		printf("claim accessory interfaced\n");
			...
			if (!send_acc) {
				...
				if (ret < 0) {
					if (ret == LIBUSB_ERROR_TIMEOUT)
						continue;
					printf("bulk in transfer error %d\n", ret);
					if (--errors == 0)
						break;
					else
						sleep(1);
				...
				acc_buf[0] = acc_buf[1] = 0x5;
				acc_buf[2] = data;
				printf("send data out ep:0x%x\n", AOA_ACCESSORY_EP_OUT);
				ret =
				libusb_bulk_transfer(acc->handle,
				...
				if (ret < 0) {
					if (ret != LIBUSB_ERROR_TIMEOUT)
						printf("bulk out transfer error %d\n", ret);
				} else {
					if (data == 1024)
						data = 30;
					else
